...
import { Package, Search, X, ArrowUpDown, MapPin, FileText, ChevronDown, ChevronUp, Truck, Store } from "lucide-react";
...
type Order = {
  id: string; userId: string; status: string; total: string; mpPaymentId: string | null; createdAt: Date;
  shippingFirstName: string | null; shippingLastName: string | null; shippingAddress: string | null;
  shippingCity: string | null; shippingProvince: string | null; shippingZip: string | null;
  shippingPhone: string | null; shippingMethod: string | null; shippingCost: string | null;
  billingType: string | null; billingName: string | null; billingDoc: string | null; billingEmail: string | null;
};
...
function ShippingBillingPanel({ order: o }: { order: Order }) {
  const hasShipping = o.shippingAddress || o.shippingFirstName;
  const hasBilling = o.billingType;
  if (!hasShipping && !hasBilling) return null;

  return (
    <div style={{ display: "grid", gridTemplateColumns: hasShipping && hasBilling ? "1fr 1fr" : "1fr", gap: 12, padding: "14px 20px", borderTop: "1px solid var(--border)", background: "var(--bg-subtle)" }}>
      {hasShipping && (
        <div style={{ padding: "12px 14px", borderRadius: "var(--radius-sm)", background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
            {o.shippingMethod === "retiro"
              ? <Store size={13} color="var(--primary)" />
              : <Truck size={13} color="var(--primary)" />}
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>
              {o.shippingMethod === "retiro" ? "Retiro en local" : "Envío a domicilio"}
            </span>
            {o.shippingCost && Number(o.shippingCost) > 0 && (
              <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--text-muted)" }}>
                ${Number(o.shippingCost).toLocaleString("es-AR")}
              </span>
            )}
            {(!o.shippingCost || Number(o.shippingCost) === 0) && (
              <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--success)", fontWeight: 700 }}>Gratis</span>
            )}
          </div>
          <p style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.7 }}>
            {o.shippingFirstName} {o.shippingLastName}<br />
            {o.shippingAddress}<br />
            {o.shippingCity}{o.shippingProvince ? `, ${o.shippingProvince}` : ""}{o.shippingZip ? ` (${o.shippingZip})` : ""}<br />
            {o.shippingPhone && <span style={{ color: "var(--text-muted)" }}>{o.shippingPhone}</span>}
          </p>
        </div>
      )}
      {hasBilling && (
        <div style={{ padding: "12px 14px", borderRadius: "var(--radius-sm)", background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.05em" }}>Facturación</span>
          </div>
          <p style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.7 }}>
            {o.billingType === "factura_a" ? "Factura A" : "Consumidor Final"}<br />
            {o.billingName && <>{o.billingName}<br /></>}
            {o.billingDoc && <span style={{ color: "var(--text-muted)" }}>CUIT/DNI: {o.billingDoc}<br /></span>}
            {o.billingEmail && <span style={{ color: "var(--text-muted)" }}>{o.billingEmail}</span>}
          </p>
        </div>
      )}
    </div>
...
export function OrdersTable({ rows }: { rows: Row[] }) {
  ...
  const [expandedShipping, setExpandedShipping] = useState<Set<string>>(new Set());
  ...
  return (
    <div>
      ...
      <div style={{ marginBottom: 20, padding: "14px 16px", borderRadius: "var(--radius)", background: "var(--bg-card)", border: "1px solid var(--border)" }}>
        <div className="admin-filters" style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ position: "relative", flex: "1 1 150px" }}>
            <Search size={14} style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)", pointerEvents: "none" }} />
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Cliente o ID..." style={{ ...inputSt, paddingLeft: 32 }} />
          </div>
          <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ ...inputSt, flex: "1 1 140px" }}>
            <option value="all">Todos los estados</option>
            {STATUSES.map(s => <option key={s.value} value={s.value}>{s.label}</option>)}
          </select>
          <input value={catSearch} onChange={e => setCatSearch(e.target.value)} placeholder="Producto..." style={{ ...inputSt, flex: "1 1 130px" }} />
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        {filtered.length === 0 ? (
          ...
          filtered.map(({ order: o, user: u, items }) => {
            const hasShippingInfo = o.shippingAddress || o.shippingFirstName;
            const isExpanded = expandedShipping.has(o.id);
            return (
              <div key={o.id} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: "var(--radius)", overflow: "hidden" }}>
                ...
                <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 12, padding: "16px 20px", borderBottom: "1px solid var(--border)" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
                    {hasShippingInfo && (
                      <button
                        onClick={() => toggleShipping(o.id)}
                        style={{ display: "flex", alignItems: "center", gap: 5, padding: "6px 10px", borderRadius: "var(--radius-sm)", border: "1px solid var(--border)", background: isExpanded ? "var(--primary-bg)" : "var(--bg-subtle)", color: isExpanded ? "var(--primary)" : "var(--text-muted)", fontSize: 12, fontWeight: 600, cursor: "pointer" }}
                      >
                        <MapPin size={12} /> Envío {isExpanded ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
                      </button>
                    )}
                ...
                {/* Shipping + Billing (expandible) */}
                {isExpanded && <ShippingBillingPanel order={o} />}
              </div>
            );
          })
        )}
